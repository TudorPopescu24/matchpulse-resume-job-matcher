...
degree_normalization_map = {
    r"\bb(\.|achelor'?s?)?\s*(of\s+(science|arts|engineering|technology|computer\s+science|application|commerce))?\b": "Bachelor",
    r"\bm(\.|aster'?s?)?\s*(of\s+(science|arts|technology|engineering|computer\s+science|application|commerce))?\b": "Master",
    r"\bp(h\.?d\.?|doctorate)\b": "PhD",
    r"\bb\.?tech\b": "Bachelor",
    r"\bm\.?tech\b": "Master",
    r"\bb\.?e\b": "Bachelor",
    r"\bb\.?e\b": "Bachelor",
    r"\bm\.?e\b": "Master",
...
def normalize_degree_text(text: str) -> str:
    text = text.lower()
    for pattern, norm in degree_normalization_map.items():
...
def rule_based_extraction(text: str) -> List[Dict]:
    matches = []
    for skill in SKILLS:
        for m in re.finditer(r'\b' + re.escape(skill) + r'\b', text):
            matches.append({"label": "Skills", "text": m.group(), "start": m.start(), "end": m.end(), "score": 1.0,
                            "source": "rule"})

    degree_pattern = r"\b(bachelor(?:'s)?|master(?:'s)?|ph\.d|phd|m\.sc|b\.sc|m\.tech|b\.tech)[^,\n]{0,100}?(degree)?[^,\n]{0,40}?(in|,)?\s+[A-Z][a-z]+(?: [A-Z][a-z]+)*"
    for m in re.finditer(degree_pattern, text, flags=re.IGNORECASE):
        matches.append({"label": "Degree", "text": m.group().strip(), "start": m.start(), "end": m.end(), "score": 1.0,
                        "source": "rule"})

    for job in JOB_TITLES:
        for m in re.finditer(r'\b' + re.escape(job) + r'\b', text):
            matches.append({"label": "Designation", "text": m.group(), "start": m.start(), "end": m.end(), "score": 1.0,
                            "source": "rule"})
    ...
    for p in year_patterns:
        for m in re.finditer(p, text.lower()):
            matches.append({
                ...
                "score": 1.0,
                "source": "rule",
                "value": extract_year_value(m.group())
            })
    ...
    for current in predictions:
        ...
        for accepted in merged:
            if not (current["end"] <= accepted["start"] or current["start"] >= accepted["end"]):
                if current["label"] == accepted["label"] and current["label"] != "Years of experience":
                    overlap = True
...
def extract_entities_from_resume(text: str) -> Dict:
    ...
    for e in entities:
        ...
        if e["label"] == "Years of experience":
            ...
            base["years_of_experience"] = e.get("years_of_experience")

        simplified[e["label"]].append(base)

    if "Skills" in simplified:
        simplified["Skills"] = link_skills_to_experience(entities)
