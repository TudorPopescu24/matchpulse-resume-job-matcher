...
from transformers import (
    ElectraTokenizerFast, ElectraForTokenClassification,
...
class ElectraForTokenClassificationWithFocalLoss(ElectraPreTrainedModel):
    def __init__(self, config, alpha=None, gamma=2.0):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.electra = ElectraModel.from_pretrained("google/electra-base-discriminator", config=config)
...
model = ElectraForTokenClassificationWithFocalLoss(config=config, gamma=2.0)
...
args = TrainingArguments(
    output_dir="/content/ner_electra",
    ...
    logging_dir="/content/logs_electra",
    report_to="wandb",
    run_name="ner_electra",
)
...
model.save_pretrained("/content/ner_electra")
tokenizer.save_pretrained("/content/ner_electra")
print("Model saved to /content/ner_electra")